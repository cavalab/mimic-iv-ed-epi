...
save_filepath <- "ps2/age/wait_times/discharged/"
...
for (age in ages) {
    ...
    to_fit <- edstays %>% filter(.data[[age]]==1 | ref_age==1)
...
for (i in 1:length(methods)) {
    for (j in 1:length(wait_times)) {
        ...
        p <- ggplot(ors, aes(y = age, x = estimate)) +
            ...
            labs(title=paste("Effect of age on likelihood of waiting more than", wait_times[j], "minutes to be discharged"), subtitle=paste0(method_names[i], ", relative to 18-29 year olds"), x = "Odds Ratios (95% CI)", y="") +
            theme_minimal() +
