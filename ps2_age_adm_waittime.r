...
#question: are WAIT TIMES affected by AGE for admitted patients?
...
adm_edstays$ref_age <- ifelse(rowSums(adm_edstays[ages])==0, 1, 0)


#create a dataframe to save odds ratios which result from different methods
na_vector <- c(rep(NA, 2*length(ages)*length(wait_times)))
odds_ratios <- data.frame(age=na_vector, wait_time=na_vector, method=na_vector, estimate=na_vector, lower_conf=na_vector, upper_conf=na_vector,
...
for (age in ages) {
    ...
    to_fit <- adm_edstays %>% filter(.data[[age]]==1 | ref_age==1)
    ...
    for (j in 1:length(wait_names)) {
        ...
        #save in table
        mcnemar_result <- c(age, wait_time, paste0("match_", threshold), as.numeric(test$estimate), as.numeric(test$conf.int[1]), as.numeric(test$conf.int[2]), as.numeric(test$p.val), length(treated_indices), length(control_indices))
        ...
        odds_ratios[index,] <- c(age, wait_time, "cov_adjustment", exp(coeffs[2]), lower_confs[2], upper_confs[2], p_values[2], num_treated, num_control)
        index <- index + 1
    }
}
...
for (i in 1:length(methods)) {
    for (j in 1:length(wait_times)) {
        ...
        p <- ggplot(ors, aes(y = age, x = estimate)) +
            ...
            labs(title=paste("Effect of age on likelihood of waiting more than", wait_times[j], "minutes to be admitted"), subtitle=paste0(method_names[i], ", relative to 18-29 year olds"), x = "Odds Ratios (95% CI)", y="") +
            theme_minimal() +
