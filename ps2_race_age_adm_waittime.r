...
save_filepath <- "ps2/race_age/wait_times/admitted/"
...
for (race in races_to_check) {
    for (age in ages[1:(length(ages)-1)]) { #exclude those in 90s

        #define the problem
        adjustment_var <- age

        #for odds ratio to make any sense, we need to limit comparison to the specified age group and the reference
        to_fit <- adm_edstays %>% filter(.data[[age]]==1 | ref_age==1, .data[[race]]==1)
    ...
    for (i in 1:length(methods)) {
        for (j in 1:length(wait_times)) {
            ...
            p <- ggplot(ors, aes(y = age, x = estimate)) +
                ...
                labs(title=paste("Effect of age on likelihood of waiting more than", wait_times[j], "minutes to be admitted for people of", race_names_to_check[k], "race"), subtitle=paste0(method_names[i], ", relative to 18-29 year olds"), x = "Odds Ratios (95% CI)", y="") +
                theme_minimal() +
