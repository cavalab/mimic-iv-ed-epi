...
gender_ors <- read.csv(paste0("ps2/gender/wait_times/discharged/wait_time_odds_ratios.csv"))
race_ors <- read.csv(paste0("ps2/race/wait_times/discharged/odds_ratios.csv"))
...
for (i in 1:length(methods)) {
    for (j in 1:length(wait_names)) {
        ...
        labs(title=paste("Odds of waiting longer than", wait_times[j], "minutes to be discharged, for women of color"), subtitle=paste0(method_names[i], ", relative to white men"), x = "Odds Ratios (95% CI)", y="") +
        theme_minimal() +
