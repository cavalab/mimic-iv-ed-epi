...
#question: are WAIT TIMES affected by RACE for admitted patients?
...
save_filepath <- "ps2/race/wait_times/admitted/"
...
for (race in races) {
    ...
    to_fit <- adm_edstays %>% filter(.data[[race]]==1 | is_white==1)
...
for (i in 1:length(methods)) {
    for (j in 1:length(wait_times)) {
        ...
        p <- ggplot(ors, aes(y = race, x = estimate)) +
            ...
            labs(title=paste("Effect of race on likelihood of waiting more than", wait_times[j], "minutes to be admitted"), subtitle=paste0(method_names[i], ", relative to white people"), x = "Odds Ratios (95% CI)", y="") +
            theme_minimal() +
