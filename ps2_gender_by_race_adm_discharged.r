...
save_filepath <- "ps2/gender/by_race/wait_times/discharged/"
...
edstays <-edstays %>% filter(time_in_ed <= 24*60, is_discharged==1)
...
for (i in 1:length(methods)) {
    for (j in 1:length(wait_times)) {
        ...
        p <- ggplot(ors, aes(y = race, x = estimate)) +
            ...
            labs(title=paste("Effect of being male on likelihood of waiting more than", wait_times[j], "minutes to be discharged, by race"), subtitle=paste0(method_names[i], ", relative to women of same race"), x = "Odds Ratios (95% CI)", y="") +
            theme_minimal() +
