...
#question: is ADMISSION affected by AGE?
...
na_vector <- c(rep(NA, 2*length(ages)))
odds_ratios <- data.frame(age=na_vector, method=na_vector, estimate=na_vector, lower_conf=na_vector, upper_conf=na_vector,
...
for (age in ages) {
    ...
    #save in table
    mcnemar_result <- c(age, paste0("match_", threshold), as.numeric(test$estimate), as.numeric(test$conf.int[1]), as.numeric(test$conf.int[2]), as.numeric(test$p.val), length(treated_indices), length(control_indices))
    ...
    odds_ratios[index,] <- c(age, "cov_adjustment", exp(coeffs[2]), lower_confs[2], upper_confs[2], p_values[2], num_treated, num_control)
    index <- index + 1
}
...
for (i in 1:length(methods)) {
    ...
    p <- ggplot(ors, aes(y = age, x = estimate)) +
        ...
        labs(title="Effect of age on likelihood of being admitted", subtitle=paste0(method_names[i], ", relative to 18-29 year olds"), x = "Odds Ratios (95% CI)", y="") +
        theme_minimal() +
