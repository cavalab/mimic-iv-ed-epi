...
na_vector <- c(rep(NA, 2*length(wait_times)*length(races)))
odds_ratios <- data.frame(race=na_vector, method=na_vector, wait_times=na_vector, estimate=na_vector, lower_conf=na_vector, upper_conf=na_vector,
